...
vec3 hash3D( vec2 p )
{
    ...
    return texture2D( iChannel0, p*0.14 ).xyz;
}
...
float voronoi( vec2 x ){
    ...
    for( int i=-1; i<=1; i++ )
    {
        ...
        vec2 ptPos = vec2(.5) + vec2(sin(time*rnd.z*1.45), cos(time*rnd.z*1.174))*.5 ; //point in local cell axis
...
void main()
{
    vec2 posnorm = gl_FragCoord.xy / resolution.xy - 0.5;
    vec4 old = texture2D(iChannel1, posnorm+0.5);
    ...
    gl_FragColor = max(old*vec4(0.96,0.985,0.995,1.), cur );
