...
vec4 marchV(in vec3 ro, in vec3 rd, in float t, in vec3 bgc) 
{
    ...
    for (int i = 0; i < 150; i++) 
    {
        ...
        col *= smoothstep(0.2 + moy * 0.05, .0, mapV(pos + 1. * lgt)) * .85 + 0.15;
        col.a *= .9;
        col.rgb *= col.a;
        rz = rz + col * (1.0 - rz.a);
...
{
    vec2 q = abs(p);
    return abs( max(max(q.x * 1.176 - p.y * 0.385, q.x * 0.727 + p.y), -p.y * 1.237) * 1.);
}
vec3 flare(vec2 p, vec2 pos) 
{
    ...
    float rz = .55 * (pow(abs(fract(a * .8 + .12) - 0.5), 3.) * (noise(a * 15.) * 0.9 + .1) * exp2((-dot(q, q) * 4.)));
