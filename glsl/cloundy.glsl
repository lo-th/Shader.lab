...
void main(){  
    ...
    vec3 rd = normalize(vec3(vUv - iResolution.xy*.5, iResolution.y*0.75)); 
    ...
    for (int i=0; i<64; i++) {
        ...
        float spec = pow( abs(max( dot( reflect(-ld, sn), -rd ), 0. )), 4.);
    ...
    // trigNoise3D(rd*1.)
    col = mix(vec3(min(col.x*1.5, 1.),  pow(abs(col.x), 2.5), pow(abs(col.x), 12.)), col, dot(cos(rd*6. +sin(rd.yzx*6.)), vec3(.333))*.25+.75);
