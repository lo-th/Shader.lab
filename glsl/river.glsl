...
void GetSurfaceInfo( Intersection intersection, out Surface surface )
{
    ...
    vec3 col = vec3(0.0);
    vec3 _sample;
    _sample = texture2D( iChannel0, intersection.m_pos.xz ).rgb;
    col += _sample * _sample * vWeights.y;
    _sample = texture2D( iChannel0, intersection.m_pos.xy ).rgb;
    col += _sample * _sample * vWeights.z;
    _sample = texture2D( iChannel0, intersection.m_pos.yz ).rgb;
