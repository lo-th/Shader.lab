...
{
    return heightMapParams.x*texture2D(iChannel0, (uv + iTime*heightMapParams.zw)*heightMapParams.y).x;
}

float bumpmap(vec2 uv)
{
    float b1 = bumpMapParams1.x*(1.0 - texture2D(iChannel0, (uv + iTime*bumpMapParams1.zw)*bumpMapParams1.y).x);
    float b2 = bumpMapParams2.x*(1.0-texture2D(iChannel0, (uv + iTime*bumpMapParams2.zw)*bumpMapParams2.x).x);
    return b1+b2;
}
