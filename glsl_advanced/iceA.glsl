...
{
    if(idx<0.) return;
    else if(idx<0.25) sliderVal[0] = saturated(val);
    else if(idx<0.50) sliderVal[1] = saturated(val);
    else if(idx<0.75) sliderVal[2] = saturated(val);
    else if(idx<1.00) sliderVal[3] = saturated(val);
}
...
{
    vec4 cPingPong = texture2D(iChannel0,vec2(0));
