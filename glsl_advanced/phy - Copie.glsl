...
vec3 ShowScene(vec3 ro, vec3 rd) {
    ...
    if (min(dstBall, dstFlr) < dstFar) 
    {
        if (dstFlr < dstBall) 
        {
            ...
            {
                vn = ObjNf(ro);
                objCol = vec4(vec3(0.7, 0.8, 0.7) * (1. - 0.4 * Fbm2(30. * vec2(dot(ro.yzx, vn), dot(ro.zxy, vn)))), 0.1);
            }
        }
 else 
        {
        ...
        col = objCol.rgb * (0.2 + 0.8 * sh * max(dot(vn, sunDir), 0.) + 0.3 * max(dot(vn, vec3(-sunDir.x, 0., -sunDir.z)), 0.)) + objCol.a * sh * pow(max(0., dot(sunDir, reflect(rd, vn))), 64.);
        col = mix(col, bgCol, clamp(3. * min(dstBall, dstFlr) / dstFar - 2., 0., 1.));
    }
 else col = bgCol;
