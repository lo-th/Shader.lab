...
//Shading
vec3 shade( in vec3 p, in vec3 n, in vec3 org, in vec3 dir, vec2 v, float s ){

    vec3 col = vec3(1.);

    if ( v.x < s ) col *= classicAmbientOcclusion( p, n, 5., .9 );
    else col *= ambientOcclusion( p, n, 5., .9 );;
...
void main(){
    ...
    vec3 r = raymarche(ro, rd, vec2(1., 30.) );
    vec3 n = normal(r.xyz);
    vec3 col = shade( r, n, ro, rd, p, s );
