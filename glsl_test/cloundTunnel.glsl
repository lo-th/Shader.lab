...
{
    vec2 mv = time * vec2(0.5, -1.0) * 0.15;
    float drh = 0.0;
    float hl = 0.0;
...
void main() {
    ...
    float sd = sin(vUv.x * 0.01 + vUv.y * 3.333333333 + time) * 1298729.146861;
    vec3 col;
    float dacc = 0.0, lacc = 0.0;
    vec3 light = vec3(cos(time * 8.0) * 0.5, sin(time * 4.0) * 0.5, ro.z + 4.0 + sin(time));
    ITSC tunRef;
    for (int i = 0; i < STP; i++) 
    {
    ...
    for (int i = 0; i < 3; i++) 
    {
        ...
        float arcseed = floor(time * 17.0 + rnd.y);
        if (arcits.dist < 20.0) 
        {
            arcits.uv *= 0.8;
