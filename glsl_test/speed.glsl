...
// https://www.shadertoy.com/view/ltt3z7

//  Sadly I couldn't use all the time I wanted for this challenge and there are still a lot of things I want to add/improve.
//  I used the Skyline shader by Otavio Good (https://www.shadertoy.com/view/XtsSWs) as the main scene.
...
vec2 Bus(vec3 baseCenter) 
{
    baseCenter *= rot_y(busAngleY);
    baseCenter *= rot_z(busAngleZ);
    baseCenter.y -= abs(0.01*busAngleZ);
    float car = udRoundBox(baseCenter + vec3(0.0, -0.022, 0.001), vec3(0.013, 0.018, 0.058), 0.0005);
    
    car = smin(car, sdBox(baseCenter + vec3(0.0, -0.0416, 0.003), vec3(0.01, 0.0004, 0.058)), -600.0);
    
    vec3 wMirror = baseCenter + vec3(0.0, -0.007, 0.0);
    wMirror.z = abs(wMirror.z)-0.03;
    float wheels = cylCap((wMirror).zyx, 0.006, 0.0145);
    
    vec2 distAndMat = vec2(wheels, 3.0);    
    distAndMat = matmin(distAndMat, vec2(car, 99999.0));    // car
    distAndMat = matmin(distAndMat, vec2(sdBox(baseCenter + vec3(0.0, -0.023, 0.00), vec3(0.014, 0.0018, 0.0579)), 99998.0));
    distAndMat = matmin(distAndMat, vec2(sdBox(baseCenter + vec3(0.0, -0.013, 0.001), vec3(0.01, 0.004, 0.059)), 99997.0));
    distAndMat = matmin(distAndMat, vec2(sdBox(baseCenter + vec3(0.0, -0.0325, 0.001), vec3(0.012, 0.005, 0.059)), 99996.0));
    distAndMat = matmin(distAndMat, vec2(sdBox(wMirror + vec3(0.0, -0.026, -0.014), vec3(0.014, 0.004, 0.01)), 99996.0));
    distAndMat = matmin(distAndMat, vec2(sdBox(wMirror + vec3(0.0, -0.026, 0.015), vec3(0.014, 0.004, 0.01)), 99996.0));
    return distAndMat;
}
...
vec2 DistanceToObject(vec3 p)
{
    ...
    vec3 p2 = p;
    rep = p;
    float carTime = animTime*0.2;  // Speed of car driving
    ...
    if (abs(fract(rep.x)-0.5) < 0.35)
    {
        ...
        rep.y *= sign(rep.z - 26.)*sign(-Repeat(p2.x - 0.5, 1.0)); // Hack to show stopped cars only in the correct lanes
    }
    ...
    frontBack *= sin(animTime*2.0 + uniqueID);
    ...
    // Bus
    distAndMat = matmin(distAndMat, Bus(p+busPos));
...
float BusAngleZ(float time)
{
    float x = 0.;
    x = mix(x, -0.26, smoothstep(4.2, 4.8, time));
    x = mix(x, 0.15, smoothstep(5., 6.5, time));
    x = mix(x, -0.15, smoothstep(6.5, 7.5, time));
    x = mix(x, 0., smoothstep(7.5, 8., time));
    x = mix(x, 0.2, smoothstep(9.6, 11.5, time));
    x = mix(x, -0.1, smoothstep(11.5, 12., time));
    x = mix(x, 0., smoothstep(12., 12.5, time));
    x = mix(x, -0.14, smoothstep(13.3, 13.8, time));
    x = mix(x, 0., smoothstep(13.8, 14.4, time));
    return x;
}

float BusAngleY(float time)
{
    float x = 1.5;
    x = mix(x, 0., smoothstep(4., 5., time));
    x = mix(x, -0.05, smoothstep(5., 6., time));    
    x = mix(x, 0.1, smoothstep(6., 7., time));
    x = mix(x, -0.05, smoothstep(7., 8., time));
    x = mix(x, 0., smoothstep(8., 9., time));
    x = mix(x, -0.22, smoothstep(9., 9.8, time));
    x = mix(x, 0., smoothstep(9.8, 10.5, time));
    x = mix(x, 0.2, smoothstep(10.5, 11.5, time));
    x = mix(x, -0.1, smoothstep(11.5, 12.5, time));
    x = mix(x, 0., smoothstep(12.5, 13., time));
    x = mix(x, -0.2, smoothstep(13., 14., time));
    x = mix(x, 0., smoothstep(14., 14.5, time));
    return x;
}

float BusPosX(float time)
{
    float center = -26.;
    float x = 10.;
    x = mix(x, 0., smoothstep(2., 5., time));
    x = mix(x, 0.2, smoothstep(5., 6.5, time));
    x = mix(x, -0.1, smoothstep(6.5, 7.4, time));
    x = mix(x, 0.1, smoothstep(7.4, 9., time));
    x = mix(x, 0.6, smoothstep(9., 10.5, time));
    x = mix(x, -0.15, smoothstep(11., 12., time));
    x = mix(x, 0.2, smoothstep(12., 13., time));
    x = mix(x, 0.9, smoothstep(13.2, 14.5, time));
    return center+x*0.13;
}

float BusPosZ(float time)
{

    float x = -1.;
    x = mix(x, -1. + (time-3.7)*.25, smoothstep(3.7, 5.4, time));
    x = mix(x, -1. + (time-3.7)*.4, smoothstep(5.4, 8., time));
    return x;
}

float adjustedTime(float time)
{
    float r = 0.;
    r = mix(r, 1.2, smoothstep(4.2, 6.25, time));
    r = mix(r, 2.4, smoothstep(17.6, 19.65, time));
    return time - r;
}
...
vec3 RayTrace(in vec2 fragCoord )
{
    ...
    // Repeat the animation after time tf
    localTime = fract(localTime / tf) * tf;
    
    
    localTime = adjustedTime(localTime);
    //localTime += t3;
    animTime = localTime;
    if (animTime > t3) animTime -= t3 - 3.;
    
    
    float animAlpha = animTime/tf;
    
    float startZ = 1.3;
    
    busAngleY = BusAngleY(animTime);
    busAngleZ = BusAngleZ(animTime);
    busPos = vec3(BusPosX(animTime),0.,BusPosZ(animTime));

    
    sunDir = normalize(vec3(0.95, 0.6, 0.2));
    ...
    if (localTime > t3)
    {
        float time = animTime - t1;
        fade = saturate(time - 3.);
        fade *= saturate(t3 - animTime);
        
        camUp = vec3(0,1,0)*rot_z(-busAngleZ);
        vec3 dir = vec3(0,0,1)*rot_y(-busAngleY+PI);
        camLookat= -busPos+dir;
        
        camPos = -busPos+0.05*normalize(dir)+camUp*0.05;
    }
    ...
    else if (animTime < t2)
    {
        ...
        camPos = vec3(25.5, 0.07+alpha*0.03, 0.84);
        
        camPos.x = mix(camPos.x, 26.+ 0.5*(26.+busPos.x), smoothstep(0.7, 1., alpha));
        camPos.z = mix(camPos.z, -busPos.z + 0.3, smoothstep(0.7, 1., alpha));               
        camUp=vec3(0,1,0);
        vec3 l = -busPos;
        camLookat= mix(vec3(190,0,90.), l, smoothstep(0.,.8, alpha));
    }
    else if (animTime < t3)
    {
        float time = animTime - t2;
        float alpha = time / (t3 - t2);
        fade *= saturate(t3 - animTime);
        
        camPos = vec3(26. + 0.5*(26.+busPos.x), min(0.12,0.1+alpha*0.03), -busPos.z + 0.3);
    ...
    if ((t <= maxDepth) || (t == alpha))
    {
        ...
        if (distAndMat.y == 99999.0)
        {
            texColor = vec3(0.1, 0.1,.1)*0.6;
        }
        else if (distAndMat.y == 99998.0)
        {
            texColor = vec3(0., 0.02,.1);          
        }
        else if (distAndMat.y == 99997.0)
        {
            texColor = vec3(0.02, 0.05,.01);          
        }
        
        else if (distAndMat.y == 99996.0)
        {
            texColor = vec3(0.);          
        }
        else if (distAndMat.y >= 100.0)
        {
