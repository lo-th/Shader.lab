...
vec2 hash22(vec2 p) { 
    ...
    //return sin( p*6.2831853 + iGlobalTime )*.35 + .65; 
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec2 a = sin(vec2(1.570796, 0) + sin(iGlobalTime/4.)*.3); // Fabrice's observation.
    ...
    // Other path.
    //float p2 = 1.0 - smoothstep(0., .1, cos(heightMap(sp.xy + 1. + iGlobalTime/4.)*6.283)*.5+.5);
    float p2 = 1.0 - smoothstep(0., .1, Voronoi(sp.xy*4. + vec2(iGlobalTime, cos(iGlobalTime/4.))));
